...
public class MetadataMediator {
  ...
  private MetadataResult<ComponentMetadataDescriptor> getMetadata(MetadataContext context, Object key) {
    ...
    List<MetadataResult<ParameterMetadataDescriptor>> inputs = inputDelegate.getParametersMetadataDescriptors(context, key);
    ComponentMetadataDescriptor componentDescriptor = componentDescriptor(component.getName())
      .withParametersDescriptor(inputs)
    ...
    //
    if (!output.isSuccess() || !inputs.stream().anyMatch(r -> !r.isSuccess())) {
      return mergeFailures(componentDescriptor,
                           (MetadataResult<?>[]) builder().add(output).addAll(inputs).build().toArray());
    }
  ...
  //                             () -> resolverFactory.getInputResolver().getInputMetadata(context, key),
   ...
   */
  private MetadataResult<ComponentMetadataDescriptor> mergeFailures(ComponentMetadataDescriptor descriptor,
