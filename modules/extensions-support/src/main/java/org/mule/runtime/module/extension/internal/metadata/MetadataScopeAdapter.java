...
public final class MetadataScopeAdapter {
  ...
  public MetadataScopeAdapter(Class<?> extensionType, Method operation, OperationDeclaration declaration) {
    ...
    inputResolvers = stream(operation.getParameters())
                      .filter(p -> p.isAnnotationPresent(TypeResolver.class))
                      .collect(toMap(NameUtils::getAliasName,
                                     p -> p.getAnnotation(TypeResolver.class).value()));

    if (outputResolverDeclaration != null || keyId.isPresent() || !inputResolvers.isEmpty()){
  ...
  public boolean isCustomScope() {
    return hasKeysResolver() ||hasOutputResolver() || hasInputResolvers();
  }
